...
Latest follow-up, 22 September around 00:51 CEST: `asitube.io` and `www.asitube.io` now pass certificate verification. HTTP `.io` returns a 301 to HTTPS; HTTPS `www.asitube.io` returns a 301 to `https://asitube.io/`. The redirect site's **Enforce HTTPS** setting persisted after reload. A normal Chrome visit to `https://asitube.io/identity.html?from=asitube-io` reached `https://asitube.com/identity.html?from=asitube-io`, preserving the path/query, then failed on the still-stale `.com` DNS lookup. The main domain continues to serve HTTPS 200 through a direct configured-IP check, but `www.asitube.com` still has a certificate mismatch. GitHub is automatically reissuing the main certificate; its HTTPS-enforcement setting remains pending. Do not unnecessarily restart provisioning.

The user explicitly approved temporary hourly follow-up for up to 48 hours. Automation `finish-asitube-domain-launch` is ACTIVE in this task. It may finish GitHub HTTPS enforcement and verify the domains, but must preserve DNS/provider settings. It should remain quiet until success or actionable failure, pause after success, or report the remaining blocker and pause by 24 September 2026 at 00:55 Europe/Berlin. The initial automatic approval rejection was resolved by that explicit user confirmation.
